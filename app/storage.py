...
class RedisStore:
    ...
    async def check_rate_limit(
        ...
        result = await self.redis.eval(
            self._RATE_LIMIT_SCRIPT, 1, key, window_seconds
        )
    ...
    def _envelope_field(envelope: Dict[str, Any]) -> str:
        return (
            f"{int(envelope['key_version'])}:"
            f"{envelope['recipient_id']}:{envelope['key_id']}"
        )
    ...
    async def add_room_keys(
        self, room_id: str, key_envelopes: Sequence[Dict[str, Any]]
    ) -> None:
